...
export async function handleWhatsappMessageAck(event = {}) {
    const updates = Array.isArray(event) ? event : [event];

    const results = [];

    for (const item of updates) {
        const result = await processAckItem(item);
...
async function processAckItem(item = {}) {
    const externalId = item.key?.id || item.externalId || null;

    if (!externalId) {
        return null;
    }

    const message = findMessageByExternalId(externalId);

    if (!message) {
        return null;
    }

    const ackStatus = item.update?.status ?? item.status ?? null;

    if (ackStatus === null || ackStatus === undefined) {
        return null;
    }

    applyAckStatus(message, ackStatus);
...
function applyAckStatus(message, ackStatus) {
    const normalizedStatus = normalizeAckStatus(ackStatus);
...
function findMessageByExternalId(externalId) {
    return messageRepository
        .list()
        .find((message) => String(message.externalId) === String(externalId));
}