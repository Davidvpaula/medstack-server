...
export function renderSecurityDashboard() {
    const report = getSecurityFinalReport();
...
<div class="header">
    ...
    <p class="small">Scanner de segurança técnico. Somente leitura.</p>
...
<div class="card">
    <h2>Resumo Geral</h2>
    <table>
        <tr><td>Scan Findings</td><td>${report.scan.summary.totalFindings}</td></tr>
        <tr><td>Risky Routes</td><td>${report.routes.summary.riskyRoutes}</td></tr>
        <tr><td>Config Pending</td><td>${report.config.summary.pending}</td></tr>
        <tr><td>Config Partial</td><td>${report.config.summary.partial}</td></tr>
    </table>
</div>
...
<div class="card full">
    ...
    ${renderRoutes(report.routes.riskyRoutes)}
</div>

<div class="card full">
    <h2>Checklist de Segurança</h2>
    ${renderChecklist(report.config.checklist)}
</div>
...
function renderRoutes(items = []) {
    if (!items.length) {
        return "<p class='small'>Nenhuma rota de risco encontrada.</p>";
    }

    return items.map((route) => `
        <div class="item">
            <strong>${route.method} ${route.path}</strong>
            <p>Módulo: ${route.module}</p>
            <p>${route.recommendation}</p>
            <p><span class="badge ${getSeverityClass(route.risk)}">${route.risk}</span></p>
            <p class="small">${route.reasons.join(" | ")}</p>
        </div>
    `).join("");
}
...
function renderChecklist(items = []) {
    return items.map((item) => `
        <div class="item">
            ...
            <span class="badge ${getChecklistClass(item.status)}">${item.status}</span>
        </div>
    `).join("");
}
...
function getStatusClass(status) {
    if (status === "secure_for_beta") return "ok";
    if (status === "needs_minor_security_work") return "warn";
    if (status === "needs_security_work") return "warn";
    return "error";
}